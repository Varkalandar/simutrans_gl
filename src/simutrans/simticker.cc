...
void ticker::add_msg(const char* txt, koord3d pos, rgba_t color, int type)
{
	node n;
...
void ticker::redraw()
{
	...
	for(node & n : list) {
		if (n.xpos < width) {
			display_proportional_clip_rgb(n.xpos, start_y + TICKER_V_SPACE, n.msg, ALIGN_LEFT, n.color, FS_NORMAL);
		}
	}
}
...
void ticker::process_click(int x)
{
	...
	if ((clicked->type & message_t::MESSAGE_TYPE_MASK) >= message_t::chat_common  &&  (clicked->type & message_t::MESSAGE_TYPE_MASK) <= message_t::chat_private) {
		...
		if (si == NULL) {
			si = new chat_frame_t();
			create_win({ 0, 200 }, si, w_info, magic_chatframe, false);
		}
