...
DBG_MESSAGE("ai_goods_t::step()","remove already constructed rail between %i,%i and %i,%i and try road",platz1.x,platz1.y,platz2.x,platz2.y);
		...
		case NR_RAIL_SUCCESS:
		{
			...
			welt->get_message()->add_message(buf, spos, message_t::ai, RGBA_WHITE, rail_engine->get_base_image());
		...
		case NR_ROAD_SUCCESS:
		{
			...
			welt->get_message()->add_message(buf, spos, message_t::ai, RGBA_WHITE, road_vehicle->get_base_image());
