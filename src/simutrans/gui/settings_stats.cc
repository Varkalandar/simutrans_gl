...
void settings_general_stats_t::init(settings_t const* const sets)
{
	...
	for(  uint32 i=0;  i<lengthof(version);  i++  ) {
		savegame.new_component<gui_scrolled_list_t::const_text_scrollitem_t>( version[i]+2, (gui_theme_t::gui_color_text) ) ;
...
void settings_climates_stats_t::init(settings_t* const sets)
{
	...
	for(  uint32 i=0;  i<lengthof(climate_generate_string);  i++  ) {
		climate_generate.new_component<gui_scrolled_list_t::const_text_scrollitem_t>( climate_generate_string[i], (gui_theme_t::gui_color_text) ) ;
	}
	...
	for(  uint32 i=0;  i<lengthof(tree_generate_string);  i++  ) {
		tree_generate.new_component<gui_scrolled_list_t::const_text_scrollitem_t>( tree_generate_string[i], (gui_theme_t::gui_color_text) ) ;
	}
