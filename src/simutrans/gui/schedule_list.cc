	...
	scl(gui_scrolled_list_t::listskin, line_scrollitem_t::compare)
{
	...
	add_table(4,0);
	{
		...
		for( int i=0; i<MAX_SORT_IDX;  i++ ) {
			sort_type_c.new_component<gui_scrolled_list_t::const_text_scrollitem_t>( translator::translate(idx_to_sort_text[i]), (gui_theme_t::gui_color_text)) ;
		}
	...
	if(  comp == &bt_new_line  ) {
		...
		if(  line_scrollitem_t *li=(line_scrollitem_t *)scl.get_element(v.i)  ) {
			...
			if(  line_info  ) {
				...
				create_win( lc, new line_management_gui_t(line, player, -1), w_info, (ptrdiff_t)line.get_rep(), false );
			}
		}
		scl.set_selection( -1 );
	}
