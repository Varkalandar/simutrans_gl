	...
	obsolete_paks(gui_scrolled_list_t::listskin)
{
	...
	for (int i = 0; i < OBSOLETE_FROM; i++) {
		if (pak_can_download(pakinfo + i)) {
			...
			paks.new_component<gui_scrolled_list_t::const_text_scrollitem_t>(pakinfo[i].name, (gui_theme_t::gui_color_text));
	...
	for (int i = OBSOLETE_FROM; i < PAKSET_COUNT; i++) {
		if (pak_can_download(pakinfo + i)) {
			paklist.append(pakinfo + i);
			obsolete_paks.new_component<gui_scrolled_list_t::const_text_scrollitem_t>(pakinfo[i].name, gui_theme_t::gui_color_text_shadow);
