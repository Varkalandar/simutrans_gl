...
bool welt_gui_t::action_triggered( gui_action_creator_t *comp,value_t v)
{
	...
	if(comp==&inp_map_number) {
		...
		if(  sg  ) {
			...
			create_win({ 10, 40 }, new settings_frame_t(sets), w_info, magic_settings_frame_t, false);
		...
		if(  climate_gui  ) {
			...
			create_win(pos, cg, w_info, magic_climate, false);
		...
		create_win({ 200, 100 }, new news_img("Erzeuge neue Karte.\n", skinverwaltung_t::neueweltsymbol->get_image_id(0)), w_info, magic_none, false);
