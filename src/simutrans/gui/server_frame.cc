...
rgba_t server_frame_t::update_info()
{
	...
	revision.set_color( engine_match ? (gui_theme_t::gui_color_text) : (MONEY_MINUS) );
	revision.update();

	pak_version.set_text( gi.get_pak_name() );
	pak_version.set_color( pakset_match ? (gui_theme_t::gui_color_text) : (SYSCOL_OBSOLETE) );
	...
	pakset_checksum.set_color( pakset_match ? (gui_theme_t::gui_color_text) : (SYSCOL_TEXT_STRONG) );
#endif
	...
	return pakset_match  &&  engine_match ? (gui_theme_t::gui_color_text) : (SYSCOL_OBSOLETE);
}
...
bool server_frame_t::update_serverlist ()
{
	...
	do {
		...
		if(  status == 1  ||  show_offline.pressed  ) {
			rgba_t color = status == 1 ? (SYSCOL_EMPTY) : (MONEY_MINUS);
