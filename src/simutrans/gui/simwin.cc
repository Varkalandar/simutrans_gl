	...
	if(  file->is_version_atleast(120, 8)  ) {
		if(  file->is_saving()  ) {
			...
			while(1) {
				...
				create_win( p, w, (wintype)win_type, id, false );
				bool sticky, rollup;
 ...
 */
static scr_coord find_no_overlap_location(gui_frame_t * const me, scr_size size)
{
    scr_coord desired_pos(0, 0); 
            
    win_clamp_xywh_position(me, desired_pos, size, true);
...
{
	return create_win({ -1, -1 }, gui, wt, magic, false);
}
...
int create_win(scr_coord pos, gui_frame_t *const gui, wintype const wt, ptrdiff_t const magic, bool move_to_full_view)
{
	...
	if(  wins.get_count() < MAX_WIN  ) {
		...
		win_clamp_xywh_position(gui, pos, window_size, move_to_full_view);
...
void win_display_flush(double konto)
{
	...
	if(  skinverwaltung_t::toolbar_background  &&  skinverwaltung_t::toolbar_background->get_image_id(0) != IMG_EMPTY  ) {
		...
		for(scr_coord_val xp=0; xp < menu_size.w; xp += env_t::iconsize.w) {
			display_base_img(back_img, menu_pos.x+xp, menu_pos.y, 0, env_t::iconsize.w, env_t::iconsize.w);
		}
	}
	else {
...
void modal_dialogue(gui_frame_t* gui, ptrdiff_t magic, karte_t* welt, bool (*quit)(), bool dismissible)
{
	...
	event_t ev;
	create_win(scr_coord(0,0), gui, w_info, magic, false);
	scr_coord pos{
