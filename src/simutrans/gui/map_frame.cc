...
class legend_entry_t : public gui_component_t
{
	...
	rgba_t color;
public:
	legend_entry_t(const char* text, rgba_t c) : label(text), color(c) {}
	...
	void draw(scr_coord offset) OVERRIDE
	{
		...
		display_fillbox_wh_clip_rgb(pos.x, pos.y+D_GET_CENTER_ALIGN_OFFSET(D_INDICATOR_BOX_HEIGHT,LINESPACE), D_INDICATOR_BOX_WIDTH, D_INDICATOR_BOX_HEIGHT, color);
...
public:
	void draw(scr_coord offset) OVERRIDE
	{
		...
		for(  int i=0;  i<MAX_SEVERITY_COLORS;  i++  ) {
			display_fillbox_wh_clip_rgb(pos.x + (scr_coord_val)(i*bar_width), pos.y+2, (scr_coord_val)bar_width+1, 7, minimap_t::calc_severity_color(i, MAX_SEVERITY_COLORS-1));
		}
	}
	scr_size get_min_size() const OVERRIDE
	{
...
void map_frame_t::draw(scr_coord pos, scr_size size)
{
	...
	if(  skinverwaltung_t::compass_map  &&  env_t::compass_map_position!=0  ) {
		...
		display_img_aligned(skinverwaltung_t::compass_map->get_image_id( isometric_img_offset+welt->get_settings().get_rotation() ),
							scrolly.get_client()+pos+scr_coord(4,4+D_TITLEBAR_HEIGHT)-scr_size(8,8), env_t::compass_map_position,
				            0, false);
	}
}
