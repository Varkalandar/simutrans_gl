...
//			assert(head_cursor_pos==tail_cursor_pos);
			...
			display_fillbox_wh_clip_rgb(x_base_offset+x_offset+start_offset, y_offset, highlight_width, LINESPACE, (SYSCOL_EDIT_BACKGROUND_SELECTED));
		...
		if(  cursor_active  ) {
			...
			if(  head_cursor_pos!= tail_cursor_pos  ) {
				...
				display_fillbox_wh_clip_rgb(x_base_offset+start_offset, y_offset, highlight_width, LINESPACE, (SYSCOL_EDIT_BACKGROUND_SELECTED));
			...
			if(  cursor_visible  ) {
				display_fillbox_wh_clip_rgb(x_base_offset+cursor_offset-1, y_offset, 1, LINESPACE, (SYSCOL_CURSOR_BEAM));
			}
		}
...
bool gui_hidden_textinput_t::infowin_event(const event_t *ev)
{
	if(  IS_LEFTRELEASE(ev)  ) {
		...
		// moreover, only (click) or (release) event happened inside textinput, the other one could lie outside
		sint16 asterix_width = display_calc_proportional_string_len_width("*", 1, 0, FS_NORMAL);
		head_cursor_pos = 0;
		if (  text  ) {
	...
	if(  text  ) {
		...
		do {
			...
			if(  cursor_visible  &&  text_pos == (utf8 const*)text + head_cursor_pos  ) {
				display_fillbox_wh_clip_rgb(xpos, pos.y+offset.y+1+(size.h-LINESPACE)/2, 1, LINESPACE, (SYSCOL_CURSOR_BEAM));
			}
