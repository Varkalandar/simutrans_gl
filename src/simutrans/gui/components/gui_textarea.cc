	...
	if (  (text != NULL)  &&  (*text != '\0')  ) {
		...
		do {
			...
			if (  -LINESPACE <= draw_y  &&  draw_y <= display_get_height() + LINESPACE) {
				...
				px_len = display_text_proportional_len_clip_rgb(x, draw_y, buf, ALIGN_LEFT | DT_CLIP, (gui_theme_t::gui_color_text), len, 0, FS_NORMAL);
			}
			else {
				// track required length when out of screen area
				px_len = display_calc_proportional_string_len_width(buf, len, 0, FS_NORMAL);
			}
			if(px_len>x_size) {
				x_size = px_len;
			}
