...
void vehiclelist_stats_t::draw( scr_coord offset )
{
	...
	int dx = display_proportional_rgb(
		...
		ALIGN_LEFT|DT_CLIP,
		veh->is_future(month) ? (SYSCOL_TEXT_HIGHLIGHT) : (veh->is_available(month) ? (gui_theme_t::gui_color_text) : gui_theme_t::gui_color_obsolete),
		false
	);
	if( veh->get_power() > 0 ) {
		char str[ 256 ];
	...
	display_multiline_text_rgb( offset.x, yyy, part1, (gui_theme_t::gui_color_text) );

	display_multiline_text_rgb( offset.x + col1_width, yyy, part2, (gui_theme_t::gui_color_text) );
}
	...
	scrolly(gui_scrolled_list_t::windowskin, vehiclelist_stats_t::compare)
{
	...
	add_table(3,0);
	{
		...
		ware_filter.new_component<gui_scrolled_list_t::const_text_scrollitem_t>(translator::translate("All"), (gui_theme_t::gui_color_text));
		...
		for (int i = 0; i < goods_manager_t::get_count(); i++) {
			...
			if (ware->get_catg() == 0) {
				ware_filter.new_component<gui_scrolled_list_t::const_text_scrollitem_t>(translator::translate(ware->get_name()), (gui_theme_t::gui_color_text));
		...
		for (int i = 1; i < goods_manager_t::get_max_catg_index(); i++) {
			...
			if (ware->get_catg() != 0) {
				ware_filter.new_component<gui_scrolled_list_t::const_text_scrollitem_t>(translator::translate(ware->get_catg_name()), (gui_theme_t::gui_color_text));
				idx_to_ware.append(ware);
			}
		}
